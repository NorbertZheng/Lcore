...
public class KeyboardCtrl implements KeyListener{
	private CP0 cp0;
	private DeviceKeyboard deviceKeyboard;
	private int interval = 1;//1,2,3
	//private int keyCode = 0;
	public KeyboardCtrl() {
		// TODO Auto-generated constructor stub
		super();
		this.deviceKeyboard = new DeviceKeyboard();
		this.cp0 = DataController.getInstance().getCp0();
	}
	@Override
	public void keyPressed(KeyEvent e) {
		...
		int c = e.getKeyCode();
		//System.out.print(c + " ");
		byte s[] = scanCodeMake[c];
		try
		{
			deviceKeyboard.WriteKeyCode(s[0]);
			cp0.SetIRC(InterruptCode.Keyboard);
			for(int i = 1; i<s.length; i++){
				Thread.sleep(interval);
				deviceKeyboard.WriteKeyCode(s[i]);
				cp0.SetIRC(InterruptCode.Keyboard);
